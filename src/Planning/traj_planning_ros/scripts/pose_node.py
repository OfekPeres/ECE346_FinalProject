...
class PoseSubscriber:
    ...
    def __init__(self):
        ...
        self.camera_subscriber = rospy.Subscriber("/nx1/zed2/zed_node/pose", PoseStamped, callback=self.callback)
        self.camera_subscriber2 = rospy.Subscriber("/nx15/zed2/zed_node/pose", PoseStamped, callback=self.callback2)
    ...
    def callback2(self, data):
        self.lock.acquire()
        self.x_traj2.append(data.pose.position.x)
        self.y_traj2.append(data.pose.position.y)
        # print(self.x_traj)
        if (len(self.x_traj2) > 200):
            self.x_traj2.pop(0)
        if (len(self.y_traj2) > 200):
            self.y_traj2.pop(0)
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        plt.scatter(listener.x_traj2, listener.y_traj2)
